...
The Krylov subspace provides the ability to extract the approximations from an m-dimensional subspace $K_m$. Since $K_m$ is the subspace of all vectors in $\mathcal{R}^n$, it can be written as $x=p(A)b$, with $p$ a polynomial of degree not exceeding $m-1$.
...
\subsubsection{Left, Right and Split Preconditioning}
...
For a linear system $Ax=b$, it is often solved by computing the factorization $A=LU$, with $L$ a lower triangular and $U$ upper triangular. One then solves efficiently $Ly=b$ and $Ux=y$ in sequence since $U$, and $L$ are all triangular.
...
often used as a preconditioner for another iterative method such as GMRES.
...
\section{GMRES Convergence Analysis and Spectrum}
...
\subsection{Type of Operations in Krylov Methods}

It is necessary to identify the main operations in Krylov methods before the parallel implementation. Consider Algorithm 2, 5, 7. We distinguish four types of operations, which are:

\begin{itemize}
	\item Matrix-vector products;
	\item AXPY operation;
	\item Dot products;
	\item Orthogonalization of vector.
\end{itemize}
...
For the parallel implementation of SpMV on modern parallel systems, the matrix with selected storage format should be distributed across different processors with considering the load balance and reduction of communications.
...
Recent years, there are several efforts to provide the parallel Krylov methods on different computing architectures. This section gives a glance at two famous software of them.
...
\subsection{The challeges of Numerical Methods}
%Global vs Local Communication, fault tolerance, multiple level parallellism, etc. 

\textcolor{red}{TO DO}

\subsubsection{Toward Extreme Computing, Some Correlated Goals}

\begin{itemize}
	\item \textbf{Minimize the global computing time};
	\item \textbf{Acclelerate the convergence};
	\item Minimize the number of communications;
	\item \textbf{Minimize the number of longer size dot products and \textit{Reduction} operations};
	\item Minimize the memory space, cache optimization;
	\item Select the best sparse matrix compressed format;
	\item Mixed arithmetic;
	\item Minimize enery consumption;
	\item \textbf{Fault tolerance, resilience}.
\end{itemize}
\subsubsection{Reduce Global Communication}

\begin{itemize}
	\item \textbf{Analysis:} Global data synchronization is expensive in distributed memory systems, especially for a hybrid platform with accelerators. When matrix A in KSMs is very sparse, the sparse matrix-vector multiplication (SpMV) invokes even more communications. Hence, strategies for reducing communication overheads in Arnoldi orthogonalization have been proposed to address this bottleneck.
	\item \textbf{Relatd works: } Sparse Matrix Storage Format \cite{montagne2004optimal, liu2015csr5,stathis2003hierarchical} and hypergraph SpMV \cite{bell2008efficient, bell2009implementing,merrill2016merge,merrill2016merge,kreutzer2014unified,ashari2014efficient,abubaker2018spatiotemporal,azad2016exploiting}, etc.
\end{itemize}

\subsubsection{Reduce Synchronization Points}

\begin{itemize}
	\item \textbf{Analysis:}
	\item \textbf{Relatd works: } Piplined GMRES \cite{ghysels2014hiding,ghysels2013hiding,rupp2016pipelined,yamazaki2017improving,ashby2012impact,sanan2016pipelined}, TSQR \cite{demmel2008communication,anderson2011communication,carson2011avoiding,chen2015tsqr} etc.
\end{itemize}

\begin{algorithm}[htbp]
	\caption{Piplined GMRES method}
	\label{alg:piplined-gmres}
	\begin{algorithmic}[1]
		\State $r_0=b-A x_0, \beta=||r_0||_2$, and $\nu_1=r_0/\beta$
		\For{$i=0,\cdots,m-1$}
		\State $z \leftarrow A\nu_i$
		\State $h_{j,i} \leftarrow \langle z,\nu_j\rangle$, $j=1,\cdots,i$
		\State \textcolor{blue}{$h_{i+1,i} \leftarrow \sqrt{||z||^2-\sum_{j=1}^{i}h^2_{j,i}}$}
		\If{$h_{i+1,i} == 0$}
		\State stop
		\Else
		\State \textcolor{blue}{$\nu_{i+1} \leftarrow (z-\sum_{j=1}^{i}h_{j,i}\nu_j)/h_{i+1,i}$}
		\EndIf
		\EndFor
		\State Compute $y_m$ which minimizes $||\beta e_1-H_m y||_2$ 
		\State $x_m=x_0+\Omega_my_m$
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=5.2in]{fig/hy-spmv.png}
	\caption{Hypergraph model for SMPV.}
	\label{hp-spmv}
\end{figure}
\fi

\subsubsection{Multi-level Parallellism}

\begin{itemize}
	\item \textbf{Analysis:}
	\item \textbf{Relatd works: }
\end{itemize}
